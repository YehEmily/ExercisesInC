...
void reduce_matrix_rows(Matrix *matrix, int i, int j) {
    if (matrix->rows[i][0] == 0) {
        printf("element 0 in row %i is already 0\n", i);
        return;
    }

    if (matrix->rows[j][0] == 0) {
        printf("err: the first element in row %i is 0\n", j);
    }

    double factor = matrix->rows[i][0] / matrix->rows[j][0];

    for (int k = 0; k < matrix->num_cols; k++) {
        matrix->rows[i][k] = matrix->rows[i][k] - (matrix->rows[j][k] * factor);
    }
}
